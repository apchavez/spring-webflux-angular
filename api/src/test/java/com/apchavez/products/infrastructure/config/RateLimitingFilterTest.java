...
class RateLimitingFilterTest {
    ...
    void should_allow_get_requests_without_calling_redis() {
        MockServerWebExchange exchange = buildExchange(HttpMethod.GET, "/api/v1/products/active", "1.1.1.1");
    ...
    void should_allow_post_when_count_is_within_limit() {
        ...
        MockServerWebExchange exchange = buildExchange(HttpMethod.POST, "/api/v1/products", "2.2.2.2");
    ...
    void should_block_post_when_redis_count_exceeds_limit() {
        ...
        MockServerWebExchange blocked = buildExchange(HttpMethod.POST, "/api/v1/products", "3.3.3.3");
    ...
    void should_block_put_when_redis_count_exceeds_limit() {
        ...
        MockServerWebExchange blocked = buildExchange(HttpMethod.PUT, "/api/v1/products/1", "4.4.4.4");
        filter.filter(blocked, passThroughChain()).block();
    ...
    void should_block_delete_when_redis_count_exceeds_limit() {
        ...
        MockServerWebExchange blocked = buildExchange(HttpMethod.DELETE, "/api/v1/products/1", "5.5.5.5");
        filter.filter(blocked, passThroughChain()).block();
    ...
    void should_track_limits_independently_per_ip() {
        ...
        for (int i = 0; i <= RateLimitingFilter.MAX_REQUESTS; i++) {
            filter.filter(buildExchange(HttpMethod.POST, "/api/v1/products", ip1), passThroughChain()).block();
        }

        MockServerWebExchange blockedIp1 = buildExchange(HttpMethod.POST, "/api/v1/products", ip1);
        ...
        MockServerWebExchange allowedIp2 = buildExchange(HttpMethod.POST, "/api/v1/products", ip2);
    ...
    void should_allow_request_when_redis_is_unavailable() {
        ...
        MockServerWebExchange exchange = buildExchange(HttpMethod.POST, "/api/v1/products", "8.8.8.8");
    ...
    void should_fall_back_to_remote_address_when_forwarded_header_is_blank() {
        ...
        MockServerHttpRequest request = MockServerHttpRequest
                .method(HttpMethod.POST, "/api/v1/products")
    ...
    void should_use_rightmost_ip_from_x_forwarded_for_header() {
        ...
        MockServerWebExchange blocked = MockServerWebExchange.from(
                MockServerHttpRequest.method(HttpMethod.POST, "/api/v1/products")
                        .remoteAddress(new InetSocketAddress("127.0.0.1", 80))
                        .header("X-Forwarded-For", spoofedIp + ", " + trustedIp)
                        .build());
        filter.filter(blocked, passThroughChain()).block();
