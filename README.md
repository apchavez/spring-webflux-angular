...
> **Dev sin instalar el toolchain:** `./docker/dev.sh api-test` / `web-test` corren dentro del mismo contenedor que usa CI, sin necesitar Java/Gradle/Node instalados localmente (`./docker/dev.sh` sin argumentos muestra las opciones).
...
|---|---|---|---|
| `POST` | `/api/v1/auth/login` | Login — credenciales demo `admin`/`DemoAdm1n#2026` (ADMIN) o `user`/`DemoUsr#2026` (USER). Devuelve un JWT | `200`, `400`, `401` |
...
| `deploy.yml` | Manual (`workflow_dispatch`) | Levanta un cluster `kind` efímero dentro del propio runner, construye las imágenes localmente, hace `helm upgrade --install`, corre un smoke test real (rollout + health checks) y destruye el cluster al final |

### Flujo de despliegue

Este proyecto de portafolio no mantiene un cluster Kubernetes vivo (sería costo idle sin beneficio). `deploy.yml` en su lugar valida el chart de punta a punta contra un cluster `kind` efímero creado en el propio runner de GitHub Actions: construye las imágenes API/web desde los Dockerfiles, las carga al cluster, instala el chart completo (Postgres, Redis, Kafka y la app corren dentro del mismo cluster efímero, sin dependencias externas), espera el rollout y hace `curl` real contra `/actuator/health/readiness` y el frontend. El cluster se destruye automáticamente al terminar el job, incluso si algo falla.

```bash
gh workflow run deploy.yml
```

No requiere secrets — es 100% autocontenido dentro del job. (El antiguo `destroy.yml` que apuntaba a un cluster remoto vía `KUBECONFIG` se eliminó: no hay cluster persistente que destruir.)

---

## Kubernetes

Asume un cluster con `ingress-nginx` y una `StorageClass` por defecto ya existentes. Para levantar uno local sin costo:

```bash
kind create cluster --name product-service
...
- Despliegue en Kubernetes vía Helm sobre cualquier cluster (local con `kind` o real), con `ingress-nginx` como dependencia externa del chart
...
Este repo forma pareja con **spring-mvc-angular**, **quarkus-react**, **net-vue**, y **spring-jpa-native**: los cinco forman la familia de proyectos de Product Management — los primeros cuatro implementan el mismo dominio (sku/name/description/category/price/stock/active), prácticamente los mismos endpoints REST (12 aquí y en spring-mvc-angular, que exponen además `GET /import/{jobExecutionId}` para consultar el estado del import CSV asíncrono; 11 en quarkus-react y net-vue, cuyo import es síncrono), el mismo tópico de Kafka `product-events` y las mismas reglas de rate limiting con Redis, con distinto stack de backend/frontend — mantenidos a propósito en paridad funcional; spring-jpa-native comparte el dominio de Product Management con un enfoque distinto, centrado en JPA/Hibernate (relaciones de entidades, fix de N+1, bloqueo optimista, auditoría) y un frontend móvil en React Native en vez de web. El trío serverless de clinic-scheduling forma un segundo grupo así, compartiendo ese otro dominio.
